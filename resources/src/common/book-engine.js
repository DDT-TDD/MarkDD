        ...
        try {
            ...
            if (puppeteer) {
                ...
                try {
                    ...
                    if (sysBrowser) {
                        launchOpts.executablePath = sysBrowser;
                    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async serve(rootDir, options = {}) {
        ...
        const requestedPort = typeof options.port === 'number' ? options.port : 4500;
        ...
        const maxPortAttempts = requestedPort > 0 ? 25 : 1;
        for (let attempt = 0; attempt < maxPortAttempts; attempt++) {
            const portToTry = requestedPort > 0 ? requestedPort + attempt : 0;
            try {
                await new Promise((resolve, reject) => {
                    ...
                    const onListening = () => {
                        ...
                        selectedPort = address && typeof address === 'object' ? address.port : portToTry;
                        resolve();
                    };
                ...
                if (error && error.code === 'EADDRINUSE' && requestedPort > 0 && attempt < maxPortAttempts - 1) {
                    continue;
                }
                throw error;
            }
        ...
        this.logger.info(`[BookEngine] Serving book at http://localhost:${selectedPort}`);
        ...
        return { port: selectedPort, outputDir: baseDir };
    }
