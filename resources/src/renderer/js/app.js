class MarkDDApp {
    constructor() {
        ...
        if (!window.MarkDDBridge) {
            console.warn('[App] MarkDDBridge is not defined - creating mock/electron fallback for tests');
            const hasRequire = typeof require !== 'undefined';
            window.MarkDDBridge = {
                isElectron: hasRequire,
                isTauri: false,
                ipcRenderer: hasRequire ? require('electron').ipcRenderer : {
                    on: () => {},
                    removeAllListeners: () => {},
                    invoke: async () => ({})
                },
                invoke: async (channel, payload) => {
                    if (hasRequire) {
                        return require('electron').ipcRenderer.invoke(channel, payload);
                    }
                    return {};
                }
            };
    ...
    async checkStartupFile() {
        try {
            ...
            const bridge = window.MarkDDBridge;

            // In Tauri mode there is no native file-open-from-system push event,
            // but in Electron it is pushed via ipcRenderer.on.
            // In Tauri mode or Electron mode listen for system push event if available
            if (bridge.ipcRenderer && typeof bridge.ipcRenderer.on === 'function') {
            ...
            // Wait for Node.js backend before making IPC calls
            if (window.MarkDDBackendReady) {
                await window.MarkDDBackendReady;
            }

            // Check for a startup file passed on the command line / file association
            const result = await bridge.invoke('get-startup-file');
            ...
            // Polling loop for single-instance file open requests (e.g. double-clicking files when app is already open)
            if (!this._startupFilePoller) {
                this._startupFilePoller = setInterval(async () => {
                    try {
            ...
            console.error('[App] checkStartupFile error:', error);
    ...
    setupElectronListeners() {
        ...
        // Clear existing listeners to prevent duplicate registrations (Electron only)
    ...
    setupMenuHandlers() {
        ...
        const adjustNestedPosition = (submenu) => {
            ...
            // If opening on the left causes the submenu to extend off the left side of the window,
            // shift it right so it remains 100% inside the visible viewport
            if (rect.left < viewportPadding) {
                const shiftX = viewportPadding - rect.left;
                nested.style.transform = `translateX(${shiftX}px)`;
                rect = nested.getBoundingClientRect();
            }
    ...
    async newBookProject(type = 'classical') {
        ...
        try {
            const result = await window.MarkDDBridge.invoke('book-init-project', {
                targetDir: rootDir,
                config: {
                    type,
                    title,
    ...
    async showBookExample(type = 'classical') {
        ...
        try {
            ...
            const tempResult = await window.MarkDDBridge.invoke('book-create-temp-example', {
                type,
    ...
    async loadBookProject() {
        ...
        try {
            ...
            const result = await window.MarkDDBridge.invoke('book-load-structure', { rootDir });
    ...
    async buildBookAll() {
        ...
        try {
            ...
            // Build HTML
            const htmlResult = await window.MarkDDBridge.invoke('book-build', { rootDir });
            ...
            // Build PDF
            const path = require('path');
            const pdfResult = await window.MarkDDBridge.invoke('book-export-pdf', {
                rootDir,
                outputPath: path.join(htmlResult.outputDir, '../book.pdf')
            });
    ...
    activateBookLivePreview(previewUrl) {
        ...
        openBtn.addEventListener('click', () => {
            ...
            try {
                window.MarkDDBridge.invoke('open-external', previewUrl).catch(() => {
                    window.open(previewUrl, '_blank', 'noopener');
                });
                return;
            } catch (error) {
                console.warn('[App] Unable to open preview externally:', error.message || error);
            }
            window.open(previewUrl, '_blank', 'noopener');        });
    ...
    async performBookSearch(query) {
        ...
        try {
            const result = await window.MarkDDBridge.invoke('book-search', { rootDir, query });
...
`;
    ...
    setupMenuBar() {
        ...
        this.bindButton('menu-exit', () => {
            window.MarkDDBridge.invoke('app-quit');
        });
    ...
    setTheme(theme) {
        ...
        // Trigger a preview re-render to update Mermaid, MathJax, code styling, etc.
        try { this.updatePreview(); } catch (e) { console.warn('Failed to update preview on theme change:', e); }
    }
...
`;
    ...
    toggleFullscreenPreview() {
        ...
        window.MarkDDBridge.invoke('toggle-fullscreen').then(result => {
            console.log('[DEBUG] toggle-fullscreen result:', result);
            if (result && result.success) {
                this.updateFullscreenButtonState(result.isFullscreen);
            }
        }).catch(error => {
            console.error('[DEBUG] toggle-fullscreen failed:', error);
        });
    }
    ...
    async initializeFullscreenState() {
        try {
            const result = await window.MarkDDBridge.invoke('get-fullscreen-state');
            if (result && result.success) {
                this.updateFullscreenButtonState(result.isFullscreen);
            }
        } catch (error) {
...
`;
    ...
    async promptImportPPTX() {
        ...
        if (!window.MarkDDBridge && typeof require === 'undefined') {
        ...
        const bridge = window.MarkDDBridge;
        try {
            const result = await bridge.invoke('show-open-pptx-dialog');
    ...
    async confirmImportPPTX() {
        ...
        try {
            ...
            const PPTXImporter = (typeof window !== 'undefined' && window.PPTXImporter) ? window.PPTXImporter : (typeof require !== 'undefined' ? require('./js/pptx-importer.js') : null);
            if (!PPTXImporter) {
                throw new Error('PPTXImporter module is not available');
            }
            const markdown = await PPTXImporter.importFile(filePath, selectedOption);
            
            const pathMock = (typeof window !== 'undefined' && window.MarkDDBridge && window.MarkDDBridge.path) ? window.MarkDDBridge.path : (typeof require !== 'undefined' ? require('path') : null);
            const fileName = pathMock ? pathMock.basename(filePath).replace(/\.pptx$/i, '.md') : filePath.split(/[/\\]/).pop().replace(/\.pptx$/i, '.md');
    ...
    async exportPresentationPPTX() {
        ...
        if (this.editor.currentFile) {
            const baseName = pathMock ? pathMock.basename(this.editor.currentFile, pathMock.extname(this.editor.currentFile)) : 'Presentation';
            defaultName = baseName + '.pptx';
            currentFileDir = pathMock ? pathMock.dirname(this.editor.currentFile) : '';
        }
        
        try {
            const result = await bridge.invoke('show-save-pptx-dialog', defaultName);
            if (result && !result.canceled && result.filePath) {
                this.showToast('Exporting to PowerPoint...', 'success');
                
                // Pre-render slides to HTML for high-fidelity PPTX export
                if (!this.presentationManager) {
                    this.presentationManager = new PresentationManager();
                }
                const parsed = this.presentationManager.parseMarkdown(markdown);
                const theme = (parsed.metadata && parsed.metadata.theme) || 'default';
                const renderedSlides = (parsed.slides || []).map((slide, idx) => ({
                    type: slide.type,
                    notes: slide.notes,
                    content: slide.content,
                    html: this.presentationManager.generateSlideHTML(slide, idx, theme)
                }));
                
                // Always invoke backend IPC for bulletproof PPTX generation via Node.js
                const exportResult = await bridge.invoke('export-presentation-pptx', {
                    markdown,
                    renderedSlides,
    ...
    async customizePresentationColors() {
        ...
        const normalizeColorMap = (map) => {
            ...
            Object.entries(map || {}).forEach(([key, value]) => {
                ...
                const trimmed = value.trim();
                if (hexPattern.test(trimmed)) normalized[key] = normalizeHex(trimmed);
            });
            return normalized;
        };
        ...
        // Build fields for the single form dialog
        const fields = [
            { id: 'primary',    label: 'Primary Color (headings, accents)', type: 'color', value: normalizedExisting.primary    || '#1976D2' },
            { id: 'secondary',  label: 'Secondary Color (highlights)',       type: 'color', value: normalizedExisting.secondary  || '#64B5F6' },
            { id: 'background', label: 'Background Color',                   type: 'color', value: normalizedExisting.background || '#FFFFFF' },
            { id: 'text',       label: 'Text Color',                         type: 'color', value: normalizedExisting.text       || '#212121' }
        ];

        const dialogResult = await this.showFormDialog({
            title: 'Customize Presentation Colors',
            message: 'Set custom color accents for the current presentation theme.',
            fields: fields,
        ...
        // Reset to defaults — remove colors block
        if (dialogResult.isReset) {
            this.updateFrontMatterColors({});
            this.showMessage('Presentation colors reset to theme defaults');
            return;
        }

        // Build updated colors map from dialog result
        const updatedColors = {};
        for (const field of fields) {
            const val = (dialogResult[field.id] || '').trim();
            if (val && hexPattern.test(val)) {
                updatedColors[field.id] = normalizeHex(val);
            }
        }

        const existingEntries = Object.entries(normalizedExisting).sort((a, b) => a[0].localeCompare(b[0]));
        const updatedEntries  = Object.entries(updatedColors).sort((a, b) => a[0].localeCompare(b[0]));
        ...
        this.updateFrontMatterColors(updatedColors);
...
`;
    ...
    async selectCVPhoto() {
        ...
        const result = await window.MarkDDBridge.invoke('select-cv-photo-dialog');
    ...
    async refreshCVPreview(focus = false) {
        try {
            const bridge = window.MarkDDBridge;
            const isOpen = await bridge.invoke('is-cv-preview-open');
            if (isOpen) {
                const markdown = this.editor.getContent();
                if (/cv:\s*true/i.test(markdown)) {
                    if (!this.cvManager) {
                    ...
                    await bridge.invoke('preview-cv', { html: html, focus: focus });
                }
            }
        } catch (e) {
    ...
    async refreshPresentationPreview(focus = false) {
        try {
            const bridge = window.MarkDDBridge;
            const isOpen = await bridge.invoke('is-presentation-preview-open');
            if (isOpen) {
                const markdown = this.editor.getContent();
                if (this.presentationManager) {
                    ...
                    await bridge.invoke('preview-presentation', { html: html, focus: focus });
                }
            }
        } catch (e) {
    ...
    handleContentChange(detail) {
        ...
        // Debounced refresh of CV and Presentation previews
        if (this.isLivePreview) {
            if (this._livePreviewDebounceTimer) {
                clearTimeout(this._livePreviewDebounceTimer);
            }
            this._livePreviewDebounceTimer = setTimeout(() => {
                this.refreshCVPreview(false);
                this.refreshPresentationPreview(false);
            }, 300); // 300ms debounce
        }
    ...
    async loadCustomThesisTemplates() {
        ...
        try {
            ...
            const userDataPath = await bridge.invoke('get-user-data-path');
            const cwd = await bridge.invoke('get-cwd');
            const appPath = await bridge.invoke('get-app-path');
    ...
    updateRecentFilesMenu() {
        ...
        recentFiles.forEach((file) => {
            ...
            button.addEventListener('pointerdown', function(e) {
                ...
                if (fileIsMissing) {
                    self.showError(`File not found at path:\n${filePath}`);
                    return;
                }
    ...
    async openRecentFileFromMenu(filePath) {
        ...
        try {
            let content = null;
            let result = await window.MarkDDBridge.invoke('read-file', { filePath });

            if (result && result.success && typeof result.content === 'string') {
                content = result.content;
            } else if (typeof result === 'string') {
                content = result;
            } else if (window.MarkDDBridge.fs && window.MarkDDBridge.fs.existsSync(filePath)) {
                content = window.MarkDDBridge.fs.readFileSync(filePath, 'utf-8');
            }

            if (typeof content !== 'string') {
                const errMsg = (result && result.error) ? result.error : 'File could not be read';
            ...
            this.showError(`Failed to open recent file: ${error.message}`);
        }
    }
    ...
    async openHelpShowcase() {
        ...
        try {
            // Wait for backend before filesystem operations
            if (window.MarkDDBackendReady) await window.MarkDDBackendReady;
            const path = require('path');
            const bridge = window.MarkDDBridge;
            let fullPath;
            const examplesResult = await bridge.invoke('get-examples-path');
            if (examplesResult && examplesResult.success) {
                fullPath = path.join(examplesResult.path, showcaseFile);
            } else {
                const cwd = await bridge.invoke('get-cwd');
                // get-cwd returns { success: true, cwd: '...' }
                const cwdPath = (cwd && cwd.success) ? cwd.cwd : '.';
                fullPath = path.join(cwdPath, showcaseFile);
            }
            const result = await bridge.invoke('read-file', fullPath);
            ...
            if (typeof content === 'string') {
                ...
                // Fallback: use inline content
                const fs = require('fs');
                if (fs.existsSync(fullPath)) {
                    const raw = fs.readFileSync(fullPath, 'utf-8');
                    await this.openFile(fullPath, raw);
                    this.showMessage('Opened Comprehensive Feature Showcase');
                } else {
    ...
    async openHelpPresentation() {
        ...
        try {
            ...
            if (examplesResult && examplesResult.success) {
                ...
                const cwd = await bridge.invoke('get-cwd');
                fullPath = path.join(cwd && cwd.success ? cwd.path : (cwd || '.'), presentationFile);
            }
            const result = await bridge.invoke('read-file', fullPath);
            const content = typeof result === 'string' ? result : (result && result.content ? result.content : null);
            if (typeof content === 'string') {
    ...
    async showAboutDialog() {
        ...
        try {
            const result = await window.MarkDDBridge.invoke('get-package-data');
            if (result && result.success) {
        ...
        // Dynamically adjust Electron/Tauri version display based on current execution environment
    ...
    async populateVersionInfo() {
        try {
            ...
            try {
                const result = await window.MarkDDBridge.invoke('get-package-data');
                if (result && result.success && result.data && result.data.version) {
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Populate version from package data after backend is ready
        (window.MarkDDBackendReady || Promise.resolve()).then(() => {
