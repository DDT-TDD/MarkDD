        ...
        * {
        ...
        @media print {
            ...
            .cv-page {
                ...
                padding: ${['modern-sidebar', 'twenty-seconds', 'hipster', 'forty-seconds', 'sixty-seconds'].includes(theme) ? '0 !important' : '20mm !important'};
            }
            .cv-page.size-letter {
                padding: ${['modern-sidebar', 'twenty-seconds', 'hipster', 'forty-seconds', 'sixty-seconds'].includes(theme) ? '0 !important' : '0.8in !important'};
            }
            .cv-header.has-photo-casual {
        ...
        if (photo && !photo.startsWith('http://') && !photo.startsWith('https://') && !photo.startsWith('data:')) {
            if (typeof require !== 'undefined') {
                try {
                    ...
                    if (typeof window !== 'undefined' && window.MarkDDBridge && typeof window.MarkDDBridge.getImageUrl === 'function' && window.MarkDDBridge.isTauri) {
                        photo = window.MarkDDBridge.getImageUrl(photo);
                    } else if (!normalizedPhoto.startsWith('file:///')) {
...
<body>
    ...
    async previewCV(markdown, currentFilePath = null) {
        ...
        if (bridge) {
            return await bridge.invoke('preview-cv', { html: html });
        }
        if (typeof require !== 'undefined') {
    ...
    async exportHTML(markdown, currentFilePath = null) {
        ...
        const bridge = (typeof window !== 'undefined' && window.MarkDDBridge) ? window.MarkDDBridge : null;
        if (bridge) {
            return await bridge.invoke('save-cv-html', {
                html: html,
    ...
    async exportPDF(markdown, currentFilePath = null) {
        ...
        const bridge = (typeof window !== 'undefined' && window.MarkDDBridge) ? window.MarkDDBridge : null;
        if (bridge) {
            return await bridge.invoke('export-cv-pdf', {
                html: html,
                title: this.metadata.name || 'CV'
            });
        }
