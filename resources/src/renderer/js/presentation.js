...
<head>
    ...
    <script src="https://cdn.jsdelivr.net/npm/@viz-js/viz@3.4.0/lib/viz-standalone.js" defer></script>
    ...
    <style>
        /* Hide MathJax assistive MML raw text */
        mjx-assistive-mml, math[aria-hidden="true"] {
...
<body>
    ...
    generateSlideHTML(slide, index, theme) {
        if (!this._slideHtmlCache) this._slideHtmlCache = new Map();
        const cacheKey = `${theme}:${index}:${slide.content}`;
        if (this._slideHtmlCache.has(cacheKey)) {
            return this._slideHtmlCache.get(cacheKey);
        }
        ...
        // NOTE: Use [\s\S]+? (not [^\$]+?) so that multiline equations with \\\\ newlines are captured
        ...
        const renderWithMathJax = (tex, display) => {
            ...
            if (typeof window !== 'undefined' && window.MathJax && typeof window.MathJax.tex2svg === 'function') {
                try {
                    ...
                    // Strip MathJax assistive MML element so raw text is never rendered below math SVGs
                    if (rendered && typeof rendered === 'string') {
                        rendered = rendered.replace(/<mjx-assistive-mml[\s\S]*?<\/mjx-assistive-mml>/gi, '');
                    }
        ...
        mathPlaceholders.forEach((value, placeholder) => {
            ...
            // Note: options is not in scope here (generateSlideHTML has no options param);
            // read engine from global markdownRenderer or default to 'mathjax'.
            const preferredEngine = (window.markdownRenderer ? window.markdownRenderer.mathEngine : null) || 'mathjax';
        ...
        const finalHtml = `    <div class="slide ${activeClass} slide-${slide.type}" data-slide-index="${index}" data-theme="${theme}"${styleAttr}>
        ${htmlContent}
    </div>`;
        this._slideHtmlCache.set(cacheKey, finalHtml);
        return finalHtml;
    }
    ...
    async exportHTML(options = {}) {
        ...
        // Use bridge to save file (works in Electron + web/Tauri modes)
        const bridge = (typeof window !== 'undefined' && window.MarkDDBridge) ? window.MarkDDBridge : null;
        if (bridge) {
            const result = await bridge.invoke('save-presentation-html', {
                html: html,
    ...
    async exportPDF(options = {}) {
        ...
        // Use bridge to export as PDF (works in Electron + web/Tauri modes)
        const bridge = (typeof window !== 'undefined' && window.MarkDDBridge) ? window.MarkDDBridge : null;
        if (bridge) {
            const result = await bridge.invoke('export-presentation-pdf', {
                html: html,
                title: this.metadata.title || 'Presentation',
    ...
    async previewPresentation() {
        ...
        // Use bridge for cross-platform compatibility (Electron + web/Tauri modes)
        const bridge = (typeof window !== 'undefined' && window.MarkDDBridge) ? window.MarkDDBridge : null;
        if (bridge) {
            const result = await bridge.invoke('preview-presentation', {
                html: html
            });
            return result;
        }
