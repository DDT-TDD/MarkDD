    ...
    async init() {
        try {
            ...
            if (typeof window !== 'undefined' && window.require) {
                ...
                console.log('[NodeTikZIntegration] IPC available for local TikZ rendering');
                this.isInitialized = true;
                console.log('[NodeTikZIntegration] Initialization successful');
                return;
            }

            // No IPC available — fall back gracefully (TikZ renders via server)
            console.warn('[NodeTikZIntegration] No IPC environment found; using HTTP backend for TikZ.');
            this.isInitialized = false;

        } catch (error) {
            console.error('[NodeTikZIntegration] Initialization failed:', error);
            this.isInitialized = false;
            // Do not re-throw — let the caller handle isInitialized === false gracefully
        }
    }
